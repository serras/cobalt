...
okRule name strictness (Env fn dat ax _) (Rx.Rule rx action) term =
  let -- 1. Add extra information for open variables
      ...
      evalWith      = Rx.eval (rule : mainTypeRules TreeScheme UseAnyType) (Rx.IndexIndependent (newEnv,[],[])) term
      evalWithout   = Rx.eval (mainTypeRules TreeScheme UseAnyType) (Rx.IndexIndependent (newEnv,[],[])) term
